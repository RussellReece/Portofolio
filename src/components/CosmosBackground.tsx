...
export default function CosmosBackground() {
  ...
  useEffect(() => {
    const initThree = () => {
      ...
      // Use fog to blend mountains into the background
      ...
      refs.renderer = new THREE.WebGLRenderer({
        ...
        antialias: true,
        alpha: true, // Transparent background to let body bg show through
      });
      ...
      const bloomPass = new UnrealBloomPass(
        ...
        0.6, // slightly reduced bloom for general background
        0.4,
        0.85
      );
    ...
    const createMountains = () => {
      ...
      const layers = [
        ...
        { distance: -150, height: 100, color: 0x0f3460, opacity: 0.5 },
        { distance: -200, height: 120, color: 0x0a4668, opacity: 0.3 },
      ];
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Define camera positions based on scroll (top to bottom)
      const startPos = { x: 0, y: 30, z: 150 };
      const endPos = { x: 0, y: 50, z: -500 };
  ...
  return (
    <canvas 
      ref={canvasRef} 
      className="fixed inset-0 w-full h-full -z-10 pointer-events-none opacity-30 dark:opacity-100 transition-opacity duration-700" 
      aria-hidden="true"
    />
  );
}
